...
export function usersReducer(state = initialUsersState, action: UserActions): UsersState {
    switch  (action.type) {
        ...
        case UserActionTypes.ProfileUpdate: return adapter.updateOne(action.payload.partialUser, state);
		...
		case UserActionTypes.UsersFormErrors : {
			return {...state , userFormErrors : action.payload.formErrors }
		}
